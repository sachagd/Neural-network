...
C_layers::C_layers(std::pair<double*, std::vector<int>> kernels, std::pair<double*, std::vector<int>> biases){
...
double * C_layers::forward(const double * inputs, int n){
    this->inputs = inputs;
    ...
    for (int i = 0; i < n; i++){
        ...
        for (int j = 0; j < this->kernels_shape[0]; j++){
            ...
            for (int k = 0; k < this->kernels_shape[1]; k++){
                add_valid_corr(ptrOutputs, ptrInputs, this->input_subshape[0], this->input_subshape[1], ptrKernels , this->kernels_shape[2], this->kernels_shape[3]);
...
double * C_layers::backward(double * input_gradient, int n, double learning_rate){
    ...
    for (int i = 0; i < n; i++){
        ...
        for (int j = 0; j < this->kernels_shape[0]; j++){
            for (int k = 0; k < this->kernels_shape[1]; k++){
                add_valid_corr(ptrKernels_gradient, ptrInputs, this->input_subshape[0], this->input_subshape[1], ptrInput_gradient, this->biases_shape[1], this->biases_shape[2]); //regarder si combiner est + efficace ici
                add_full_conv(ptrOutput_gradient, ptrInput_gradient, this->biases_shape[1], this->biases_shape[2], ptrKernels, this->kernels_shape[2], this->kernels_shape[3]);
                ptrKernels += this->kernels_subsize;
    ...
    sub_mul(this->kernels, kernels_gradient, learning_rate / n, this->kernels_size);
    sub_mul_mean(this->biases, input_gradient, learning_rate, n, this->biases_size);
    return output_gradient;
}

Fc_firstlayers::Fc_firstlayers(std::pair<double*, std::vector<int>> weights, std::pair<double*, std::vector<int>> biases){
...
double * Fc_firstlayers::forward(double * inputs, int n){
    this->inputs = inputs;
    this->output_not_activated = add2(dot1(inputs, this->weights, n, this->weights_shape[0], this->weights_shape[1]), this->biases, n, this->weights_shape[1]);
    return maximum(this->output_not_activated, 0, n * this->weights_shape[1]);
}

double * Fc_firstlayers::backward(double * input_gradient, int n, double learning_rate){
    mul_deriv_relu(input_gradient, this->output_not_activated, n * this->weights_shape[1]);
    double * output_gradient = dot2(input_gradient, this->weights, n, this->weights_shape[1], this->weights_shape[0]);
    sub_mul(this->weights, dot1(transpose(this->inputs, n, this->weights_shape[0]), input_gradient, this->weights_shape[0], n, this->weights_shape[1]), learning_rate / n, this->weights_size);
    sub_mul_mean(this->biases, input_gradient, learning_rate, n, this->weights_shape[1]);
    return output_gradient;
}

Fc_lastlayer::Fc_lastlayer() : weights(nullptr), biases(nullptr) {}

Fc_lastlayer::Fc_lastlayer(std::pair<double*, std::vector<int>> weights, std::pair<double*, std::vector<int>> biases){
    this->weights = weights.first;
    this->weights_shape = weights.second;
    this->weights_size = multiplication(this->weights_shape);
    this->biases =  biases.first;
...
double * Fc_lastlayer::forward(double * inputs, int n){
    this->inputs = inputs;
    double * output_not_activated = add2(dot1(inputs, this->weights, n, this->weights_shape[0], this->weights_shape[1]), this->biases, n, this->weights_shape[1]);
    return softmax(output_not_activated, n, this->weights_shape[1]);
}

double * Fc_lastlayer::backward(double * y_pred, double * y_true, int n, double learning_rate){
    double * input_gradient = sub(y_pred, y_true, n * this->weights_shape[1]);
    double * output_gradient = dot2(input_gradient, this->weights, n, this->weights_shape[1], this->weights_shape[0]);
    sub_mul(this->weights, dot1(transpose(this->inputs, n, this->weights_shape[0]), input_gradient, this->weights_shape[0], n, this->weights_shape[1]), learning_rate / n, this->weights_size);
    sub_mul_mean(this->biases, input_gradient, learning_rate, n, this->weights_shape[1]);
    return output_gradient;
...
void NeuralNetwork::create(std::vector<int> input_shape, std::vector<std::vector<int>> kernel_shape, std::vector<int> fcnpc){
...
void NeuralNetwork::train(double learning_rate, int n, int nb, int nbbt){
...
double * NeuralNetwork::forpropagation(double * input, int n){
...
void NeuralNetwork::backpropagation(double * outputs, double * labels, int n, double learning_rate){
...
double NeuralNetwork::accuracy(double * acc_outputs, double * test_images, int * test_labels){
    acc_outputs = forpropagation(test_images, 10000); //regarder ici, pq j'envoie acc_outputs en argument de la fonction
    int ind;
    double max;
    double acc;
